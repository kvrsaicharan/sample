...
module.exports = {
  ...
  getUserProperties: function (options) {
    ...
    return {
      ...
      token: {
        ...
        authenticationType: token.ext_attr ? 'xsuaa' : 'ias',
  ...
  getCallBackUrl: function (req) {
    ...
    var loginCallback = multipleAuthTypes(req) ? req.routerConfig.appConfig.login.callbackEndpoint + '?authType=' + getAuthenticationType(req) :
  ...
  loadOauthOptions: function (req, cb) {
    var authenticationType = getAuthenticationType(req);
...
function getAuthenticationType(req){
  var authenticationType = pathUtil.pathAuthenticationType(req);
  var urlObj = req.url ? urijs.parse(req.url) : null;
  if (urlObj && urlObj.path.indexOf(req.routerConfig.appConfig.login.callbackEndpoint) > -1 && urlObj.query){
    var authType = querystring.parse(urlObj.query).authType;
